            ...
            <form method="POST" action="{{ route('dashboard.books.memo', $book->id) }}" class="max-w-sm mx-auto min-w-12">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="text"
                           ...
                           placeholder="Book Title" value="{{ $book->name }}" name="name" required/>
                </div>
                <div class="mb-5">
                    <label for="amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Number of
                        books</label>
                    <input type="number" id="amount" name="amount" value="{{ $book->amount }}"
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    ...
                    <div>
                        ...
                        <select id="type" name="type"
                            ...
                            @forelse($types as $type)
                                <option value="{{ $type->id }}" @if($type->id == $book->id) selected @endif >{{ $type->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        <a>
                    ...
                    <div>
                        ...
                        <select id="category" name="category"
                            ...
                            @forelse($categories as $category)
                                <option value="{{ $category->id }}" @if($category->id == $book->id) selected @endif>{{ $category->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        <a>
                    ...
                    <div>
                        ...
                        <select id="authors" name="authors"
                            ...
                            @forelse($authors as $author)
                                <option value="{{ $author->id }}" @if($category->id == $book->id) selected @endif>{{ $author->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        <a>
