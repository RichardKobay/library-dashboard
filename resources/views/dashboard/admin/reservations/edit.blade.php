<x-app-layout xmlns="">
    ...
    <div class="flex justify-center items-center mt-12 flex-col">
        <div
            ...
            <form method="POST" action="{{ route('dashboard.admin.reservations.update', $reservation->id) }}" class="max-w-sm mx-auto min-w-12">
                @csrf
                @method('put')
                <div class="mb-5">
                    <div>
                        ...
                        <select id="type" name="user_id"
                            ...
                            @forelse($users as $user)
                                <option value="{{ $user->id }}" @if($user->id == optional($reservation->user)->id) selected @endif>{{ $user->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        <a>
                    ...
                    <div>
                        ...
                        <select id="type" name="book_id"
                            ...
                            @forelse($books as $book)
                                <option value="{{ $book->id }}" @if($book->id == optional($reservation->book)->id) selected @endif>{{ $book->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        <a>
