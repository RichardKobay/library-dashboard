<x-app-layout xmlns="">
    ...
    <div class="flex justify-center items-center mt-12 flex-col">
        <div
            ...
            <form method="POST" action="{{ route('dashboard.admin.loans.update', $loan->id) }}" class="max-w-sm mx-auto min-w-12">
                @csrf
                @method('put')
                <div class="mb-5">
                    <div>
                        ...
                        <select id="type" name="user_id"
                            ...
                            @forelse($users as $user)
                                <option value="{{ $user->id }}" @if($user->id == optional($loan->user)->id) selected @endif>{{ $user->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        <a>
                    ...
                    <div>
                        ...
                        <select id="type" name="book_id"
                            ...
                            @forelse($books as $book)
                                <option value="{{ $book->id }}" @if($book->id == optional($loan->book)->id) selected @endif>{{ $book->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        <a>
                ...
                <div class="mb-5">
                    ...
                    <input type="date" id="date" name="date" value="{{ $loan->date }}"
                ...
                <div class="mb-5">
                    ...
                    <input type="date" id="return_date" name="return_date" value="{{ $loan->rerurn_date }}"
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                           placeholder="YYYY-MM-DD" required/>
                </div>
                <button type="submit"
